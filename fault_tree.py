...
class BasicEvent(Event):
    ...
    def to_xml(self, printer):
        ...
        printer.write('<define-basic-event name="'+ self.name+ '">')
        printer.write('<float value="'+ str(self.prob)+ '"/>')
        printer.write('</define-basic-event>')

    def to_aralia(self, printer):
        """Produces the Aralia definition of the basic event."""
        printer.write('p(', self.name, ') = '+ self.prob)
...
class HouseEvent(Event):
    ...
    def to_xml(self, printer):
        ...
        printer.write('<define-house-event name="'+ self.name+ '">')
        printer.write('<constant value="'+ self.state+ '"/>')
        printer.write('</define-house-event>')

    def to_aralia(self, printer):
        """Produces the Aralia definition of the house event."""
        printer.write('s(', self.name, ') = '+ self.state)
...
class Gate(Event):  # pylint: disable=too-many-instance-attributes
    ...
    def to_xml(self, printer, nest=False):
        ...
        printer.write('<define-gate name="'+ self.name+ '">')
        printer.write(convert_formula(self, nest))
        printer.write('</define-gate>')
...
class CcfGroup:  # pylint: disable=too-few-public-methods
    ...
    def to_xml(self, printer):
        ...
        printer.write('<define-CCF-group name="'+ self.name+ '"'+ ' model="'+
                self.model+ '">')
        printer.write('<members>')
        for member in self.members:
            printer.write('<basic-event name="'+ member.name+ '"/>')
        printer.write('</members>')

        printer.write('<distribution>')
        printer.write('<float value="'+ str(self.prob) + '"/>')
        printer.write('</distribution>')

        printer.write('<factors>')
        assert self.model == "MGL"
        assert self.factors
        level = 2
        for factor in self.factors:
            printer.write('<factor level="'+ str(level) + '">')
            printer.write('<float value="'+ str(factor) + '"/>')
            printer.write('</factor>')
            level += 1
        printer.write('</factors>')

        printer.write('</define-CCF-group>')
...
class FaultTree:  # pylint: disable=too-many-instance-attributes
    ...
    def to_xml(self, printer, nest=False):
        ...
        """
        printer.write('<opsa-mef>')
        printer.write('<define-fault-tree name="'+ self.name+ '">')
        ...
        printer.write('</define-fault-tree>')

        printer.write('<model-data>')
